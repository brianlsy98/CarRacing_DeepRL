...
class Agent:
    ...
    def train(self, tracklist, e):
        ...
        figure_file = self.model_path+'rewards.png'
        ...
        map_random_choose = 1
        map_not_finished = list(np.array(tracklist))
        first_world = tracklist[0]

        for i in range(n_games):
            # Get initial state
            # ====== Initialization  ====== #
            if len(map_not_finished) == 0:
                print("all tracks completed once")
                if map_random_choose == 1:
                    world_name = random.choice(tracklist)
                obs = env.reset(name = world_name)
            elif len(map_not_finished) == len(tracklist):
                world_name = first_world
                obs = env.reset(name = world_name)
            elif map_random_choose == 1:
                world_name = random.choice(map_not_finished)
                obs = env.reset(name = world_name)
            elif map_random_choose == 0:
                obs = env.reset(name = world_name)

            print(tracklist)
            print(map_not_finished)
            print(world_name)
            ...
            while not done:
                ...
                if (env.get_pose().position.z) > 0.0007 : done = True
                # if collision : reward -500
                if done == True and info['info'] != 3 :
                    reward -= 500; map_random_choose = 0

                # if checkpoint reached fast, reward ++
                # reward += info['checkpoints']
                if done == True and info['info'] == 3 :
                    reward += 500; map_random_choose = 1
                    if len(map_not_finished) != 0:
                        map_random_choose = 1
                        map_not_finished.pop(map_not_finished.index(world_name))
                    # laptime_history.append(rospy.get_time() - START_TIME)

                obs_norm_, _, _ = self.obs_normalize(obs_[0])

                # reward -= 300*abs(np.mean(obs_norm_[3:8])-np.mean(obs_norm_[19:24]))
                # reward += obs_norm_[12]
                reward += (input_velocity-1.5)**2
                reward -= 0.5*abs(np.mean(obs_norm_[:13])- np.mean(obs_norm_[13:26]))
            ...
            # Save by laptime
            # if len(laptime_history) > 0:
            #     laptime = laptime_history[-1] 
            #     if laptime < best_laptime or len(laptime_history) < 3:
            #         if len(laptime_history) < 3 : best_laptime = 100
            #         else : best_laptime = laptime
            #         self.save_models()
